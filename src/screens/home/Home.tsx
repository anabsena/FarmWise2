...
export function Home() {
  ...
  useEffect(() => {
    ...
    const fetchClimaAtual = async () => {
      try {
        const response = await fetch(
          `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${cidadePesquisada}`
        );
        const data = await response.json();
        setClimaAtual(data);
      } catch (err) {
    ...
    const fetchPrevisaoSemanal = async () => {
      try {
        const response = await fetch(
          `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${cidadePesquisada}&days=7`
        );
        const data = await response.json();
    ...
    async function fetchCotacoesGranos() {
      ...
      try {
        const response = await fetch(
          `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbols}&apikey=${apiKeyAlphaVantage}`
        );
        const data = await response.json();
        ...
        const promises = symbols.map((symbol) =>
          fetch(
            `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=${apiKeyAlphaVantage}`
          ).then((response) => response.json())
        );
        const results = await Promise.all(promises);
        const cotacoes = results.reduce((acc, result, index) => {
          const symbol = symbols[index];
          const price = result["Global Quote"]
  ...
  const getIconByCondition = (condition: string, className: string) => {
    ...
    const conditionIcons = {
      ...
      "Partly Cloudy ": (
        <img src="/img/parcialmente-nublado.svg" className={className} />
      ),
      ...
      "Light thunderstorm": (
        <img src="/img/trovoada.svg" className={className} />
      ),
      "Heavy thunderstorm": (
        <img src="/img/trovoada.svg" className={className} />
      ),
  ...
  const handleAtualizarCotacoesGranos = async () => {
    ...
    try {
      const promises = symbols.map(async (symbol) => {
        const response = await fetch(
          `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${symbol}&apikey=${apiKeyAlphaVantage}`
        );
        const data = await response.json();
        ...
        if (data.Information) {
          ...
          throw new Error(
            `Dados de cotação para ${symbol} inválidos. Limite de uso gratuito atingido.`
          );
        }
      ...
      console.error(err);
      setError(
        "Erro ao atualizar cotações dos grãos. Limite de uso gratuito atingido."
      );
    }
  };
      ...
      <div className=" w-full grid grid-cols-1 md:grid-cols-2 h-full relative">
        <div className="mt-4 p-8">
          <form
            ...
            className="flex flex-col gap-4 w-full justify-center items-center mb-4"
          >
          ...
          {climaAtual && (
            <div>
              <p className="text-primary text-xl ">
                {" "}
                {conditionTranslations(climaAtual.current.condition.text)}
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                {getIconByCondition(
                  climaAtual.current.condition.text,
                  "text-4xl text-secondary w-52"
                )}
                <div>
                  <h2 className="uppercase text-6xl font-bold text-primary">
                    {cidadePesquisada}
                  </h2>
                  <p
                    className="text-center text-2xl"
                    dangerouslySetInnerHTML={{ __html: formatCurrentDate() }}
                  ></p>
                  <p className="bg-secondary p-2 text-white flex justify-center text-6xl font-bold relative rounded-xl">
                    <span className="font-normal text-[12px] absolute top-2 left-2">
                      Agora
                    </span>{" "}
                    {climaAtual.current.temp_c}°C
                  </p>
                  <p className="uppercase font-bold text-xl text-center">
                    Umidade: {climaAtual.current.humidity}%
                  </p>
                </div>
              </div>
            </div>
                ...
                <div key={index} className="flex flex-col items-center">
                  ...
                  <div className="flex flex-col items-center border-2 p-4 border-secondary w-full rounded-xl h-full justify-between">
                    {getIconByCondition(
                      ...
                      "width-24 text-secondary w-24"
                    )}
                    <p className="text-center text-sm text-primary">
                      {" "}
                      {conditionTranslations(dia.day.condition.text)}
                    </p>
                    <p className="bg-secondary p-1 text-white rounded-lg">
                      {" "}
                      {dia.day.maxtemp_c}°C - {dia.day.mintemp_c}°C
                    </p>
                    <p></p>
                  </div>
                </div>
        ...
        <div className="container mx-auto mt-10 relative h-full">
          <div className="p-4 rounded flex flex-col items-end">
            <div className="flex items-center ">
              <h2 className="text-4xl uppercase font-bold text-primary mb-2">
                Cotação do dólar
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 ">
              <div className="p-4 rounded flex flex-col justify-center items-center z-50">
                ...
                <p className="text-xl">
                  ...
                  {cotacoes.soja ? cotacoes.soja.toFixed(2) : "Carregando..."}
                </p>
              </div>
              <div className="p-4 rounded flex flex-col justify-center items-center z-50">
                <div className="flex items-center">
                ...
                <p className="text-xl">
                  <span className="font-bold text-secondary">R$</span>
                  {cotacoes.milho ? cotacoes.milho.toFixed(2) : "Carregando..."}
                </p>
              </div>
              <div className="p-4 rounded flex flex-col justify-center items-center z-50">
                <div className="flex items-center">
                ...
                <p className="text-xl">
                  <span className="font-bold text-secondary">R$</span>
                  {cotacoes.trigo ? cotacoes.trigo.toFixed(2) : "Carregando..."}
                </p>
              </div>
            </div>
          ) : (
          ...
          <img
            src="/img/moeda.svg"
            className="absolute bottom-0 right-0 opacity-60 z-0 hidden md:flex"
            alt=""
          />
        </div>
      </div>
